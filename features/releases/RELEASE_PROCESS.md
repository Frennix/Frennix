...
13. **Physical device QA for new UI (permanent).** No new UI component, overlay, or interaction pattern is **complete** until tested on a **physical iPhone in Safari** (and native app when applicable) for scrolling, safe areas, animations, responsiveness, and usability. Simulator and automated checks alone are insufficient. Record results in the release checklist or bug list before marking the feature done.
14. **Design system compliance (permanent).** All new bottom sheets, modals, menus, and interactions must follow [`DESIGN_SYSTEM.md`](../../DESIGN_SYSTEM.md) and [`FRENNIX-DESIGN-SYSTEM.md`](./FRENNIX-DESIGN-SYSTEM.md) — spacing, radius, typography, motion, and safe-area tokens. No one-off overlay implementations. See also [`DEVELOPMENT_STANDARDS.md`](../../DEVELOPMENT_STANDARDS.md).
15. **Stabilize before features (permanent).** Fix freezes, lag, viewport, scroll, rendering, and Safari regressions on a physical iPhone **before** starting major new features. Performance optimization and UI polish come next. Documented in [`DEVELOPMENT_STANDARDS.md`](../../DEVELOPMENT_STANDARDS.md).
